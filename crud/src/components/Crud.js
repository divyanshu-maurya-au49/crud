...
function Crud() {
  const data1 = {name:"",lastname:"",email:"",password:"",project:""}
    ...
    const handelSubmit= async(e)=>{
      ...
      try {
       const {data} =  await axios.post('/api/v1/user/send',{name:input.name, lastname:input.lastname,email:input.email,password:input.password,project:input.project})
      if(data.success){
...
<div className="container">
  ...
  {tabledata.map((item,i)=>(
      ...
      <td>{item.project}</td>
      <td>
        <button onClick={()=>handelEdit(i)}  className='btn btn-primary'>Edit</button>
        <button onClick={()=>handelDelete(i)} className='btn btn-danger'>Delete</button>
      </td>
    </tr>
  ))}
